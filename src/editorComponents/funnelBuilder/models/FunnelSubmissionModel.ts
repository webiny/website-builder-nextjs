...
export class FunnelSubmissionModel {
  ...
  async submitActiveStep(): Promise<FunnelSubmissionStepSubmissionResult> {
    ...
    // Get the next step.
    const nextStepIndex = this.funnel.steps.findIndex(step => step.id === this.activeStepId) + 1;
  ...
  getFieldsForActiveStep() {
    ...
    return Object.values(this.fields).filter(field => field.definition.stepId === activeStep.id);
  }
